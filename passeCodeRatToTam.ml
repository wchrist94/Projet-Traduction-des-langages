      ...
      begin
        match e with
            ...
            |AstType.Ident (iast) -> 
                begin
                    match (info_ast_to_info iast) with
                        (* Chargement de la variable dans le registre avec la bonne taille
                           grâce aux informations contenues dans l'InfoVar *)
                        |InfoVar (_,t,d,r) -> 
                              load (getTaille t) d r
                        (* Chargement de l'entier contenue dans l'InfoConst *)
                        |InfoConst (_, i) -> 
                              loadl_int i
                        (* Erreur si appel de fonction effectué sans call cas impossible
                           sauf erreur interne *)
                        |_ -> 
                              failwith "erreur"
                end
                ...
                begin
                    match (info_ast_to_info iast) with
                        (* Accès à la taille, la valeur et le registre de la variable
                           grâce à son InfoVar *)
                        |InfoVar (_, t, d, r) ->
                            (* Génération du code lié à l'affectation le code de
                               l'expression est généré par analyse_code_expression *)
                            (analyser_code_expression e) 
                            ^ (store (getTaille t) d r)
                        (* Autre type d'info impossible sauf erreur interne *)
                        |_ -> failwith "erreur"
