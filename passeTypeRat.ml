			...
			begin
						match i with
									...
									|AstTds.Affectation(iast, e) -> 
												(match info_ast_to_info iast with
															(* Accès au type de la variable *)
															|InfoVar (_,t,_,_) -> 
																	(* Transformation de l'expression associée à l'affectation et accès à son type *)
																	let (ne, te) = analyser_type_expression e in
																			(* Vérification que le type de l'expression correspond à celui contenu dans l'info *)
																			if (est_compatible te t) then
																					(* Renvoie un AstType.Affectation contenant l'info et la nouvelle expression *)
																					AstType.Affectation (iast, ne)
																			(* Si le type de l'expression n'est pas conforme on renvoie une erreur 
																				 indiquant le type attendu *)
																			else
																					raise (TypeInattendu (te, t))
															(* Erreur si on souhaite réaliser une affectation sur autre chose qu'une variable ce qui est
																 un cas impossible sauf erreur interne *)
															|_ -> failwith "erreur interne")
									|AstTds.Affichage e -> 
